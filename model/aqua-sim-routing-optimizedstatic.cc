#include "aqua-sim-routing-optimizedstatic.h"
...
int set_Finish_Initial_Network = 0;
...
{
    static TypeId tid =
        TypeId("ns3::AquaSimOptimizedStaticRouting")
            ...
            .AddAttribute("packet_time_out",
                          ...
                          DoubleValue(400),
                          MakeDoubleAccessor(&AquaSimOptimizedStaticRouting::m_pkt_t2_out),
                          MakeDoubleChecker<double>())
            .AddAttribute("broad_timeout_T1_s",
                          "Time out of packets Aggregation",
                          DoubleValue(200),
                          MakeDoubleAccessor(&AquaSimOptimizedStaticRouting::m_pkt_t1_out),
                          MakeDoubleChecker<double>())
            ...
            .AddTraceSource(
                "optimizedInitialNode",
                "Node:finish initial network",
                MakeTraceSourceAccessor(&AquaSimOptimizedStaticRouting::m_InitialNodeTrace),
                "AquaSimOptimizedStaticRouting::InitialNodeCallback");
    return tid;
}
...
AquaSimOptimizedStaticRouting::ReadRouteTable(char* filename)
{
    ...
    if (set_Finish_Initial_Network == 0)
    {
        set_Finish_Initial_Network = 1;
        std::vector<int> if_lonely(m_NodeNum, 1);
        ...
        while (!feof(stream))
        {
            ...
            {
                m_rTable.insert({AquaSimAddress(dst_node),
                                 {AquaSimAddress(nxt_hop1),
                                  AquaSimAddress(nxt_hop2)}}); // 在multimap中存储两个下一跳
                NS_LOG_UNCOND("Node("
                              << AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt()
                              << "): Received route table : " << AquaSimAddress(dst_node).GetAsInt()
                              << "    " << AquaSimAddress(nxt_hop1).GetAsInt() << "    "
                              << AquaSimAddress(nxt_hop2).GetAsInt());
            }
            if (nxt_hop1 != -1)
                if_lonely[current_node - 1] = 0;
        }
...
AquaSimOptimizedStaticRouting::Recv(Ptr<Packet> p, const Address& dest, uint16_t protocolNumber)
{
    ...
    if (!has_set_neighbor)
    {
        hashTable.AquaSimGPSRHashTable::SetUp(
            AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),
            m_NodeNum,
            m_initial_neighbor_string);
        packet_table.AquaSimGPSRPktTable::SetUp(
            AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),
            m_NodeNum);
        has_set_neighbor = true;
        hashTable.CheckNeighborTimeout(
            AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),
            m_neighbor_t_out,
            m_neighbor_t_slot,
            m_NodeNum);
        packet_table.CheckBroadcastpktTimeout(
            AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),
            m_pkt_t2_out,
            m_pkt_t_slot);
    }
    ...
    if (static_cast<int>(ash.GetNetDataType()))
    {
        ...
        double time_finish =
            hashTable.PutInHash(p,
                                AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),
                                m_NodeNum);
        if (time_finish != 0)
        {
            NS_LOG_UNCOND("Finish initial network at " << time_finish);
            m_InitialNodeTrace(AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt(),time_finish);
            if (Finish_Initial_Network == m_NodeNum)
            {
                m_InitialNetTrace(time_finish);               
                NS_LOG_UNCOND("!!!!!!all nodes find their initial neighbors at " << time_finish);
            }
        }
        m_rxTrace(p,
                  Simulator::Now().GetSeconds(),
...
AquaSimOptimizedStaticRouting::FindNextHop(const Ptr<Packet> p)
{
    ...
    hashPtr = hashTable.AquaSimGPSRHashTable::GetHash(
        AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt());

    std::multimap<AquaSimAddress, std::pair<AquaSimAddress, AquaSimAddress>>::iterator it =
        m_rTable.find(dst);
    // 检查是否找到了元素
    if (it == m_rTable.end())
    {
    ...
    if (nxt_hop1 == AquaSimAddress(-1)) // 表示下一跳是-1，即两个节点不连通
    {
        return nxt_hop_fail;
    }
    for (int i = 0; i < m_NodeNum; i++)
    {
        NS_LOG_UNCOND("Node(" << AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt()
                              << ") neighbor " << i + 1 << " is alive?" << hashPtr->alive[i]);
    }
    if ((hashPtr->alive[nxt_hop1.GetAsInt() - 1] == 1) ||
        (nxt_hop1.GetAsInt() == AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt()))
        find1 = 1;
    if (nxt_hop2.GetAsInt() < 1000) // 如果nexthop2=-1，那么是65535
    {
        if ((hashPtr->alive[nxt_hop2.GetAsInt() - 1] == 1) ||
            (nxt_hop2.GetAsInt() == AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt()))
            find2 = 1;
    }
    NS_LOG_UNCOND("nxt_hop1  " << nxt_hop1.GetAsInt() << " find1  " << find1 << "  nxt_hop2  "
                               << nxt_hop2.GetAsInt() << " find2  " << find2);
    ...
    {
        NS_LOG_UNCOND("Node(" << AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt()
                              << ") 选择了备选下一跳节点 " << nxt_hop2.GetAsInt() << " 节点 ");
        return nxt_hop2;
    }
    else
        return nxt_hop_fail;
...
{
    NS_LOG_FUNCTION(this);
    for (std::map<int, gpsr_neighborhood*>::iterator it = neighbor_table.begin();
         it != neighbor_table.end();
         ++it)
    {
        delete it->second;
    }
...
AquaSimGPSRHashTable::SetUp(int my_node, int total_nodes, std::string initial_neighbor_string)
{
    ...
    gp->neighbor_position.resize(total_nodes);
    gp->t_now.resize(total_nodes);
    gp->alive.resize(total_nodes);
    gp->find_initial_num = 0;
    ...
    std::copy(std::begin(YourInitialNeighbor),
              std::end(YourInitialNeighbor),
              std::begin(gp->alive));
    NS_LOG_UNCOND("Node(" << my_node << ") has " << gp->initial_num << " initial neighbors");
...
AquaSimGPSRHashTable::PutInHash(Ptr<Packet> p, int my_node, int total_nodes)
{
    ...
    {
        // 在初始邻居表里的邻居
        NS_LOG_UNCOND("Node(" << my_node << ") find initial neighbor node(" << src.GetAsInt()
                              << ") at " << Simulator::Now().GetSeconds());
        hashPtr->neighbor_position[i] = nowposition;
        hashPtr->t_now[i] = Simulator::Now();
        hashPtr->alive[i] = 1;
        hashPtr->neighbor_num++;
        hashPtr->find_initial_num++;
        if ((hashPtr->find_initial_num == hashPtr->initial_num) && (hashPtr->initial_num != 0))
        {
            Finish_Initial_Network++;
            NS_LOG_UNCOND("!!!Node(" << my_node << ") find all initial neighbors at "
                                     << Simulator::Now().GetSeconds());
    ...
    {
        NS_LOG_UNCOND("Node(" << my_node << ") find new neighbor node(" << src.GetAsInt() << ") at "
                              << Simulator::Now().GetSeconds());
        hashPtr->neighbor_position[i] = nowposition;
        hashPtr->t_now[i] = Simulator::Now();
        hashPtr->alive[i] = 1;
...
void
AquaSimGPSRHashTable::CheckNeighborTimeout(int my_node, double Tout, double Tslot, int total_nodes)
{
    ...
    for (int i = 0; i < total_nodes; i++)
    {
        ...
        {
            NS_LOG_UNCOND("Node(" << my_node << ")'s Neighbor " << i + 1
    ...
    if (update == 1)
        NS_LOG_UNCOND("Node(" << my_node << ") has " << neighborhood->neighbor_num
    ...
    Simulator::Schedule(time_slot,
                        &AquaSimGPSRHashTable::CheckNeighborTimeout,
                        this,
                        my_node,
                        Tout,
                        Tslot,
...
AquaSimGPSRPktTable::SetUp(int my_node, int total_nodes)
{
    ...
    std::vector<Time> new_t(300, Seconds(9000000));
    gpkt->pkt_uid.assign(300, UINT16_MAX); // 不初始化就报错
    gpkt->pkt_t = new_t;
    gpkt->pkt_lastsend_t = new_t;
    gpkt->pkt_num = 0;
    gpkt->pkt_bitstring.assign(300, UINT16_MAX);
...
AquaSimGPSRPktTable::PutInPkt(Ptr<Packet> p,
                              AquaSimHeader ash,
                              int my_node,
                              Time t1_time_out,
                              Time t2_time_out)
{
    ...
    if (it != my_pkt.end())
    {
        ...
        { // 超过聚合时间的包，更新bitstring，并广播
            hashPkt->pkt_lastsend_t[i] = Simulator::Now();
            return 0;
        }
    }
    else
    {
        NS_LOG_UNCOND("it is a new packet, its uid is " << pktuid
                                                        << ", so this packet be forwarded");
        int num = hashPkt->pkt_num;
        hashPkt->pkt_uid.push_back(pktuid);
        hashPkt->pkt_t.push_back(Simulator::Now());
        hashPkt->pkt_lastsend_t.push_back(Simulator::Now());
        hashPkt->pkt_bitstring.push_back(ash.GetSendBitString());
        hashPkt->pkt_num++;
...
AquaSimGPSRPktTable::CheckBroadcastpktTimeout(int my_node, double Tout, double Tslot)
{
    ...
    Time t2_time_out = Time::FromDouble(Tout, Time::S);
    Time time_slot = Time::FromDouble(Tslot, Time::S);
    ...
    { // check for nullptr
        ...
        {
            if (Simulator::Now() - packet->pkt_t[i] > t2_time_out)
            {
                // NS_LOG_UNCOND("Packet " << packet->pkt_uid[i] << " is deleted from
                // AquaSimGPSRPktTable because of time out");
                //  交换当前超时的元素与末尾元素
        ...
        // if (updated)  NS_LOG_UNCOND("Node(" << nodeid << ") has " << packet->pkt_num << " packets
        // after updating ");
    }
    // 重新调度自己
    Simulator::Schedule(time_slot,
                        &AquaSimGPSRPktTable::CheckBroadcastpktTimeout,
                        this,
                        my_node,
                        Tout,
                        Tslot);
...
bool
AquaSimOptimizedStaticRouting::CheckNeighborSent(uint64_t bitstring)
{
    int my_node = AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt();
    uint64_t mask = 0;
    gpsr_neighborhood* neighborhood;
    neighborhood = hashTable.GetHash(my_node);
    int update = 0;

    for (int i = 0; i < m_NodeNum; i++)
    {
        if (neighborhood->alive[i] == 1)
        {
            mask |= (1 << i);
        }
    }
...
// 获取存的包的bitstring
uint64_t
AquaSimOptimizedStaticRouting::UpdateBitStringInList(Ptr<Packet> p, uint64_t bitString)
{
    int my_node = AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt();
    ...
    gpsr_packet* packet;
    packet = packet_table.GetPkt(my_node);
    if (packet)
    { // check for nullptr
        bool updated = false;
        for (size_t i = 0; i < packet->pkt_uid.size();)
        {
            if (pktuid == packet->pkt_uid[i])
            {
                packet->pkt_bitstring[i] |= bitString;
                return packet->pkt_bitstring[i];
            }
            else
            {
                ++i;
