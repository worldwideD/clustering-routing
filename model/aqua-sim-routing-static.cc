...
AquaSimStaticRouting::GetTypeId()
{
  static TypeId tid = TypeId ("ns3::AquaSimStaticRouting")
    ...
    .AddAttribute("SetRouteTableFile",
                    ...
                    StringValue(""),
                    MakeStringAccessor(&AquaSimStaticRouting::m_routeFile_string),
                    MakeStringChecker())
  ;
  return tid;
}
...
AquaSimStaticRouting::ReadRouteTable(char *filename)
{
  ...
  //printf("open routing table file successfully\n");
	while( !feof(stream) ) {
...
AquaSimStaticRouting::Recv (Ptr<Packet> p, const Address &dest, uint16_t protocolNumber)
{
  ...
  if(!m_hasReadRouteFile){
    ReadRouteTable((char*)m_routeFile_string.c_str());
    m_hasReadRouteFile = true;
  }
  AquaSimHeader ash;
  p->PeekHeader(ash);
  //NS_LOG_UNCOND("Me(" << AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt() << "): Received packet from MAC : " << ash.GetSize() << " bytes ; " << ash.GetTxTime().GetSeconds() << " sec. ; Dest: " << ash.GetDAddr().GetAsInt() << " ; Src: " << ash.GetSAddr().GetAsInt() << " ; Next H.: " << ash.GetNextHop().GetAsInt());
  if (IsDeadLoop (p))
    {
  ...
  if (AmIDst (p))
    {
      NS_LOG_UNCOND("Node("<< AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt() << "): I am destination. Sending up.");
      SendUp (p);
      return true;
    }
  ...
  NS_LOG_UNCOND("Node(" << AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt() << "): Received packet from MAC : " << ash.GetSize() << " bytes ; " << ash.GetTxTime().GetSeconds() << " sec. ; Dest: " << ash.GetDAddr().GetAsInt() << " ; Src: " << ash.GetSAddr().GetAsInt() << " ; Next H.: " << next_hop.GetAsInt());
  ...
  if (next_hop != AquaSimAddress::GetBroadcast ())
  ...
  else
    {
      ...
      NS_LOG_INFO("Dropping packet " << p << " due to no broadcast");
...
AquaSimStaticRouting::FindNextHop(const Ptr<Packet> p)
{
  AquaSimHeader ash;
  p->PeekHeader(ash);
  std::map<AquaSimAddress, AquaSimAddress>::iterator it = m_rTable.find(ash.GetDAddr());//查找路由表以确定数据包应该发送到的下一跳地址，如果找不到合适的路由，则返回一个广播地址。
