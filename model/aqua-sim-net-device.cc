...
AquaSimNetDevice::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::AquaSimNetDevice")
            ...
            .AddAttribute("SetCx",
                          "Set x for VBF related protocols.",
                          DoubleValue(0),
                          MakeDoubleAccessor(&AquaSimNetDevice::m_cX),
                          MakeDoubleChecker<double>())
            .AddAttribute("SetCy",
                          "Set y for VBF related protocols.",
                          DoubleValue(0),
                          MakeDoubleAccessor(&AquaSimNetDevice::m_cY),
                          MakeDoubleChecker<double>())
            .AddAttribute("SetCz",
                          "Set z for VBF related protocols.",
                          DoubleValue(0),
                          MakeDoubleAccessor(&AquaSimNetDevice::m_cZ),
                          MakeDoubleChecker<double>())
            .AddAttribute("SetFailureStatus",
                          "Set node failure status. Default false.",
                          BooleanValue(0),
                          MakeBooleanAccessor(&AquaSimNetDevice::m_failureStatus),
                          MakeBooleanChecker())
            .AddAttribute("SetFailureStatusPro",
                          "Set node failure status pro.",
                          DoubleValue(0),
                          MakeDoubleAccessor(&AquaSimNetDevice::m_failureStatusPro),
                          MakeDoubleChecker<double>())
            .AddAttribute("SetFailurePro",
                          "Set node failure pro.",
                          DoubleValue(0),
                          MakeDoubleAccessor(&AquaSimNetDevice::m_failurePro),
                          MakeDoubleChecker<double>())
            .AddAttribute("NextHop",
                          "Set next hop. Default is 1.",
                          IntegerValue(1),
                          MakeIntegerAccessor(&AquaSimNetDevice::m_nextHop),
                          MakeIntegerChecker<int>())
            .AddAttribute("SinkStatus",
                          "Set the sink's status, int value.",
                          IntegerValue(0),
                          MakeIntegerAccessor(&AquaSimNetDevice::m_sinkStatus),
                          MakeIntegerChecker<int>())
            .AddAttribute("SetAttacker",
...
AquaSimNetDevice::DoDispose(void)
{
    ...
    m_macSync = 0;
    m_macLoc = 0;
    // m_routing=0;  //FIXME in some cases this will lead to seg fault bug (smart pointer is deleted
    // somewhere else leading to a unref issue)
    m_node = 0;
    m_uniformRand = 0;
    m_energyModel = 0;
    m_attackModel = 0;
    m_ndn = 0;
    for (std::vector<Ptr<AquaSimChannel>>::iterator it = m_channel.begin(); it != m_channel.end();
         ++it)
        *it = 0;
    NetDevice::DoDispose();
}
...
AquaSimNetDevice::SetRouting(Ptr<AquaSimRouting> routing)
{
    ...
    if (m_routing == 0)
    {
        ...
        // liucm20240906 set forwardup callback
...
// liucm
void
AquaSimNetDevice::ForwardUp(Ptr<Packet> pkt, uint16_t protocolNumber, const Mac8Address& src)
{
    NS_LOG_FUNCTION(this);
...
AquaSimNetDevice::SendWithHeader(Ptr<Packet> packet, uint16_t protocolNumber)
{
    ...
    { // Note : https://www.nsnam.org/docs/release/3.24/doxygen/uan-mac-cw_8cc_source.html#l00123
        ...
        NS_LOG_DEBUG("Me(" << AquaSimAddress::ConvertFrom(GetAddress()).GetAsInt()
                           << "): Sending packet to Routing layer : " << ash.GetSize()
                           << " bytes ; " << ash.GetTxTime().GetSeconds() << " sec. ; Dest: "
                           << ash.GetDAddr().GetAsInt() << " ; Src: " << ash.GetSAddr().GetAsInt()
                           << " ; Next H.: " << ash.GetNextHop().GetAsInt());
    ...
    else if (MacEnabled() && m_mac)
    {
        ...
        packet->AddHeader(ash);
        NS_LOG_DEBUG("Me(" << AquaSimAddress::ConvertFrom(GetAddress()).GetAsInt()
                           << "): Sending packet to MAC layer : " << ash.GetSize() << " bytes ; "
    ...
    else if (m_phy)
    {
        ...
        NS_LOG_DEBUG("Me(" << AquaSimAddress::ConvertFrom(GetAddress()).GetAsInt()
                           << "): Sending packet to Phy layer : " << ash.GetSize() << " bytes ; "
                           << ash.GetTxTime().GetSeconds() << " sec. ; Dest: "
                           << ash.GetDAddr().GetAsInt() << " ; Src: " << ash.GetSAddr().GetAsInt()
                           << " ; Next H.: " << ash.GetNextHop().GetAsInt());
...
bool
AquaSimNetDevice::SendFrom(Ptr<Packet> packet,
