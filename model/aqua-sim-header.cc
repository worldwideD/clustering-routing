 ...
 */
 ...
 {
     static TypeId tid =
         TypeId("ns3::AquaSimHeader").SetParent<Header>().AddConstructor<AquaSimHeader>();
     return tid;
 }
 ...
 AquaSimHeader::Deserialize(Buffer::Iterator start)
 {
     ...
     m_net_packet_type = i.ReadU8();
     m_bitString = i.ReadU64();
     m_time_slot = i.ReadU64();
 ...
 AquaSimHeader::GetSerializedSize(void) const
 {
     ...
     return (8 + 2 + 1 + 2 + 1 + 2 + 2 + 2 + 2 + 4 + 4 + 1 + 8 + 8);
 }
 ...
 AquaSimHeader::Serialize(Buffer::Iterator start) const
 {
     ...
     i.WriteU8(m_net_packet_type);
     i.WriteU64(m_bitString);
     i.WriteU64(m_time_slot);
 }
 ...
 uint32_t
 AquaSimHeader::GetSeqNum()
 {
     return m_seq;
 }
 
 uint8_t
 AquaSimHeader::GetNetDataType()
 {
     return m_net_packet_type;
 }
 
 uint64_t
 AquaSimHeader::GetSendBitString()
 {
     return m_bitString;
 }
 
 uint64_t 
 AquaSimHeader::GetTimeSlot()
 {
     return m_time_slot;
 }
 ...
 void
 AquaSimHeader::SetNetDataType(uint8_t net_packet_type)
 {
     m_net_packet_type = net_packet_type;
 }
 
 void
 AquaSimHeader::SetSendBitString(uint64_t bitString)
 {
 ...
 void
 AquaSimHeader::SetTimeSlot(uint64_t time_slot)
 {
 ...
 {
     static TypeId tid =
         TypeId("ns3::AquaSimPacketStamp").SetParent<Header>().AddConstructor<AquaSimPacketStamp>();
     return tid;
 }
 ...
 AquaSimPacketStamp::Serialize(Buffer::Iterator start) const
 {
     ...
     i.WriteU8(m_status);
     i.WriteU64((uint64_t)(m_txDistance * 1000.0));
 }
 
 void
 AquaSimPacketStamp::Print(std::ostream& os) const
 {
     os << "PacketStamp: Pt(" << m_pt << ") Pr(" << m_pr << ") TxRange(" << m_txRange << ") Freq("
        << m_freq << ") Noise(" << m_noise << ") PacketStatus(";
     switch (m_status)
     {
     case RECEPTION:
         os << "RECEPTION";
 ...
 double
 AquaSimPacketStamp::GetTxDistance()
 {
     return m_txDistance;
 }
 ...
 void
 AquaSimPacketStamp::SetTxDistance(double distance)
 {
     m_txDistance = distance;
 }
