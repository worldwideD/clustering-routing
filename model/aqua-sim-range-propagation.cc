               ...
               std::vector<Ptr<AquaSimNetDevice> > dList)
{
  ...
  Ptr<DolphinMobilityModel> senderModel = sObject->GetObject<DolphinMobilityModel> ();
  ...
  for(; it != dList.end(); it++, i++)
  {
    ...
    Ptr<DolphinMobilityModel> recvModel = rObject->GetObject<DolphinMobilityModel> ();
    /*
    if (std::fabs(recvModel->GetPosition().x - senderModel->GetPosition().x) > pstamp.GetTxRange())
      break;
    */
   dist = senderModel->GetRealDistanceFrom(recvModel);
    // if ( dist > pstamp.GetTxRange() && pstamp.GetTxRange() != -1)
    //   continue;

		pru.recver = dList[i];
		pru.pDelay = Time::FromDouble(dist / AcousticSpeed(std::fabs(recvModel->GetRealPosition().z - senderModel->GetRealPosition().z)),Time::S);
    ...
    << ") pR(" << pru.pR << ")" << " Pt(" << pstamp.GetPt() << ")" << senderModel->GetRealPosition() << " & " << recvModel->GetRealPosition());
	}
	return res;
}
