 ...
 */
 ...
 class AquaSimHeader : public Header
 {
   public:
     ...
     Time GetTimeStamp();
     uint32_t GetSeqNum();
     uint8_t GetNetDataType();
     uint64_t GetSendBitString();
     ...
     void SetNetDataType(uint8_t net_packet_type);
 
     void SetSendBitString(uint64_t bitString);
     void SetTimeSlot(uint64_t time_slot);
   ...
   private:
     ...
     addr_t m_src;
     addr_t m_dst;
     uint8_t m_errorFlag; // 当前的m_errorFlag分为几类？
     uint16_t m_uId;
     uint16_t m_size; // figmented size of packet...
     Time m_timestamp;
     uint32_t m_seq;
     uint8_t m_net_packet_type; // 0正常包，1位置包
     uint16_t m_bitString;
     uint64_t m_time_slot;
 }; // class AquaSimHeader
 ...
 class AquaSimPacketStamp : public Header
 {
   public:
     ...
     void SetTxDistance(double distance);
