...
//liucm20240906
void
AquaSimRouting::SetForwardUpCb(Callback<void, Ptr<Packet>, uint16_t, const Mac8Address&> cb)
{
    m_forwardUpCb = cb;
}
...
AquaSimRouting::SendUp(Ptr<Packet> p)
{
  ...
  */
  //liucm20240906 add
  uint16_t protocolNumber = 0; //temp set 0
  //this callback use mac8address=m_address[0]
  //   p->RemoveHeader(ash);
  /*
  ...
  //  p->Print(std::cout);*/
  m_forwardUpCb(p, protocolNumber, ash.GetSAddr().ConvertToMac8Address()); 
...
AquaSimRouting::AmISrc(const Ptr<Packet> p)
{
  ...
  p->PeekHeader(asHeader);
  //return (asHeader.GetSAddr()==AquaSimAddress::ConvertFrom(m_device->GetAddress())) && (asHeader.GetNumForwards() == 0);
  return (asHeader.GetNumForwards() <= 0);
}
...
AquaSimRouting::AmIDst(const Ptr<Packet> p)
{
  ...
  AquaSimHeader asHeader;
  p->PeekHeader(asHeader);
  return (asHeader.GetDAddr() == AquaSimAddress::ConvertFrom(m_device->GetAddress()));
...
AquaSimRouting::AmINextHop(const Ptr<Packet> p)
{
  ...
  AquaSimHeader asHeader;
  p->PeekHeader(asHeader);
  //NS_LOG_UNCOND("AmINextHop??? Next Hop: " << asHeader.GetNextHop().GetAsInt()<<" Node: "<<AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt());
  return ((asHeader.GetNextHop().GetAsInt() == AquaSimAddress::ConvertFrom(m_device->GetAddress()).GetAsInt())|| ( asHeader.GetNextHop() == AquaSimAddress::GetBroadcast() ));
}
