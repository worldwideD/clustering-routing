...
#define COMPONET_NAME "Dolphin_Scenario_1"
                           ...
                           DolphinConfig* config)
{
    ...
    if (index < nodes.size())
    {
        ...
        else
        {
            ...
            if (config->mobility_config.mobilityEndTimeList.size() > 0)
            {
            ...
            {
                uint16_t rxTxType = DolphinApplication::RECV;
                double send_data_starttime = config->app_config.send_start_s;
                if (nodeId == config->senders[0])
                {
                    rxTxType |= DolphinApplication::SEND_BROADCAST;

                    send_data_starttime = CalculateNextSlotTime(
                        config->app_config.send_start_s,
