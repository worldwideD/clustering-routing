...
#define COMPONET_NAME "Dolphin_Clustering_Only_Forming"
                           ...
                           DolphinConfig* config)
{
    ...
    if (index < nodes.size())
    {
        ...
        else
        {
            ...
            double startTime = config->simulation_time_s + 50;
            for (int sendNodeId : config->senders)
            {
                createApplication(config,
                                  nodes.at(sendNodeId),
                 ...
                 ++i)
            {
                ...
                if (asDevice)
                {
                    ...
                    if (n->GetId() < config->node_num * config->long_nodes_rate)
                    {
                        ...
                        rt->SetAttribute("Range", DoubleValue(config->phy_config.maxRange));
                    }else
                    {
                        rt->SetAttribute("Ability", StringValue("SHORT"));
                        rt->SetAttribute("Range", DoubleValue(config->phy_config.relRange));
                    }
                    rt->Initialize();
                }
