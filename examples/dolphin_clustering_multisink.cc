...
#define COMPONET_NAME "Dolphin_Clustering_Sink"
        ...
        else
        {
            ...
            while (startTime < config->simulation_time_s)
            {
                for (uint16_t sender : config->senders)
                {
                    ...
                    {
                        if (startTime < config->simulation_time_s && sender != receiver)
                        {
            ...
            while (startTime < config->simulation_time_s)
            {
                ...
                // for (uint16_t sender : config->senders)
                //     for (uint16_t receiver = 0; receiver < config->sink; receiver++)
                //     {
                //         txrx.push_back(std::make_pair(sender, receiver));
                //     }
                 ...
                 ++i, ++cur)
            {
                ...
                if (asDevice)
                {
                    ...
                    if (n->GetId() < config->sink + (config->node_num - config->sink) * config->long_nodes_rate)
                    {
                        ...
                        config->net_config.m_routingProtocol == "ns3::RoutingALRP")
                        {
                            rt->SetAttribute("Range", DoubleValue(config->phy_config.longRange));
                        }         
                    }else
                    {
                    ...
                    // if (config->net_config.m_routingProtocol == "ns3::DolphinRoutingClustering")
                    // {
                    //     rt->SetAttribute("NumSinks", IntegerValue(config->sink));
                    // }
                    if (config->net_config.m_routingProtocol == "ns3::DollphinRoutingClustering")
                    {
                        if (n->GetId() == 0)
                            rt->SetAttribute("Identity", StringValue("SINK"));
                    }
                    else if (n->GetId() < config->sink && config->net_config.m_routingProtocol != "ns3::RoutingPAUV")
                    {
                        rt->SetAttribute("Identity", StringValue("SINK"));
                    }else if (config->net_config.m_routingProtocol == "ns3::RoutingRCRP")
                    {
                        Ptr<RoutingRCRP> rcrp = rt->GetObject<RoutingRCRP>();
                        for (uint16_t sink = 0; sink < config->sink; sink++)
                        {
                            rcrp->AddSinkPos(AquaSimAddress::ConvertFrom(nodes.at(sink)->GetDevice(0)->GetAddress()),
                                             config->net_config.sinkPos[sink]);
                        }
                    }
                    rt->Initialize();
                }
