    ...
    for runNum in range(args.runs):
        ...
        for key, val in data.items():
            if key == "node_positions":
                ...
                # outputs['simulation_time'] = 100 + 20 * (sink) * (nodenum - sink)
                outputs['simulation_time'] = 100 + 10000
        ...
        if model == 'ALRP':
            outputs['prob'] = 7
            outputs['routing_time'] = 1
        outputs['sink'] = sink
        outputs['senders'] = [i+1 for i in range(sink, nodenum)]
        outputs['receivers'] = [i+1 for i in range(sink)]
        ...
        command = [
            "./ns3", "run",
            f'dolphin_clustering_multisink'
        ...
        traceFile = f'trace_{runNum+1}_1000_0_{nodenum}_{sink+1}_1_dolphin_scenario.txt'
...
def main():
    ...
    # for model in [args.models, 'SOAM', 'LEACH', 'ALRP']:
    for model in ['LEACH', 'ALRP']:
        print(f'model = {model}')
        values = [['maxSpeed',
               'nodenum',
        ...
        for speed in [5]:
            ...
            for nodenum in nodenums:
                ...
                if model != args.models:
                    hello = 30
                if speed >= 1:
                    result = Simulation(model, args.long_rate, nodenum, speed, hello, 40, 1.0, [0.4, 0.2, 0.4], [0.5, 0.5], [0.5, 0.5], 3, args)
                else:
                    result = Simulation(model, args.long_rate, nodenum, 100, hello, 40, 0, [0.4, 0.2, 0.4], [0.5, 0.5], [0.5, 0.5], 3, args)  
                vals = [speed, nodenum]
                for v in result.values():
                    vals.append(v)
                values.append(vals)
        ...
        workbook.save(f'result_sink_{model}.xlsx')
