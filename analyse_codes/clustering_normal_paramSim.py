...
def main():
    ...
    # for model in [args.models, 'PAUV', 'LEACH']:
    for model in ['LEACH']:
        ...
        for speed in [5]:
            ...
            for nodenum in nodenums:
                ...
                if model != args.models:
                    hello = 30
                if speed >= 1:
                    result = Simulation(model, args.long_rate, nodenum, speed, hello, 40, 1.0, [0.4, 0.2, 0.4], [0.5, 0.5], [0.5, 0.5], 1, args)
                else:
                    result = Simulation(model, args.long_rate, nodenum, 100, hello, 40, 0, [0.4, 0.2, 0.4], [0.5, 0.5], [0.5, 0.5], 1, args)  
                vals = [speed, nodenum]
                for v in result.values():
                    vals.append(v)
                values.append(vals)
